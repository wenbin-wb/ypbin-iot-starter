...
## 一之二、CI 与依赖获取的现状（发布前必须解决）

| 项 | 现状 |
|---|---|
| 远端仓库 | **未配置**（本地 clone 无 remote）→ `.github/workflows/` 的三个工作流**从未执行过** |
| 父 pom 获取 | 父版本是 `3.1.0-SNAPSHOT`，**未发布到远程**；CI 里没有「先构建母仓」的步骤，因此 CI 目前**必然卡在父 POM 解析** |
| 处置 | ① 配置远端并让 CI 能拿到母仓（例如先把母仓发布 3.1.0 正式版，再把本仓父版本改钉正式版）；② 在此之前，工作流的绿灯**不构成验证** |
