...
**重连/重绑定已落地（2026-09-14）—— 本里程碑最大的架构缺口已闭合**

此前 `reconnect-initial-delay` / `reconnect-max-delay` / `reconnect-jitter` 三项配置**零消费**：
链路被判死后条目被摘除、会话被关闭、订阅被取消，而框架只在启动与设备变更时 bind →
**一次网络抖动 = 该设备永久离线**。所有「检测断开」「误判」类修复的后果都因此被永久化。

| 设计点 | 取舍与理由 |
|---|---|
| 退避**按 `connectionId` 独立** | 一台 Modbus 网关挂 200 个从站共享一条链路。用全局退避会让一条坏链路拖慢所有链路的恢复节奏；独立维护才能「只让坏的那条慢下来」 |
| 抖动只向**更长**方向 | 比基准更早重试会让退避失去意义（对端还没恢复就又被敲一次）；抖动的作用是打散集群惊群 |
| `schedule` **幂等** | 同链路已在重连中时不重复排定，避免「一次断开触发 N 次重连风暴」 |
| 失败日志**按链路限流**（30s） | 链路长期不可用时不能刷屏——与已修的「丢弃日志风暴」同类考量 |
| 重连必须**整体成功**才算恢复 | 一台设备绑定失败说明链路仍不可用；「部分恢复」会让退避节奏与实际可用性脱节 |
| 显式关闭 / 设备 REMOVE **必须取消重连** | 否则会把已解绑的设备重新绑回来 |
| 主动关闭（`CLIENT_REQUEST`）**不重连** | 停机与人工解绑不应触发恢复 |

**实现过程中被端到端测试抓出的两个真缺陷（均已修）**：

1. **重连把自己取消了**：`reconnect → bind → 先 unbind 再 bind`，而当时的 `unbind` 会清除设备归属并
   在链路无设备时取消重连 → 重连动作刚发出就把自己的状态删掉，`totalRecovered` 恒为 0。
   修复：拆成 `detach`（仅断开会话/句柄，供内部重绑用）与 `unbind`（公开语义＝detach + 清除归属 + 按需取消重连）。
2. **测试断言时机错误**：以 `sessionCount()==1` 作为等待条件，而它在重连前就成立（旧会话尚未释放），
   导致断言跑在退避到期之前。改为以**恢复计数**为等待条件。

> 这两条再次印证同一件事：**覆盖率达标不等于功能可用**。若不写端到端重连用例，
> 「重连把自己取消」这个缺陷会以「配置项存在但永不恢复」的形态安静地留在仓库里。

**宿主回调已卸载出协议线程（2026-09-14）—— I4 从「文档承诺」变为「代码约束」**

此前 `DataListener.onData` 在**协议库的回调线程上同步执行**：
MQTT 是 HiveMQ 的 IO 线程、OPC UA 是 Milo 的 **JVM 全局共享执行器**
（实测 core=0/max=Integer.MAX_VALUE，被同 JVM 所有 Milo 连接复用）、TCP 是 Netty EventLoop、
轮询订阅是调度器的**单一定时器线程**。宿主只要在回调里做一次落库或 HTTP 调用（现场极常见），
就会阻塞整条链路：一条 Modbus 网关链路上的 200 个从站、或同 JVM 内所有 Milo 连接一起被拖住。
DESIGN §5.4 承诺的 `ThreadIdentityGuard` 防线**全仓并不存在**。
...
覆盖 4 处调用点：`PollingSubscriptionManager`（轮询订阅）、`MqttSession.dispatch`、
...
**④ 分支覆盖率门禁已落地（2026-09-14）**

在父仓 `INSTRUCTION ≥ 0.80` 之上，本仓额外加了一条 **BRANCH ≥ 0.64** 下限。
...
未覆盖的分支几乎全是并发交错、饱和/关闭边界与幂等分支，正是风险所在。

**两次踩坑（都靠反向验证发现）**：

1. 第一版把规则放在 `pluginManagement` 里 → **完全不生效**（阈值改 0.99 仍 BUILD SUCCESS）。
   原因：父仓是在 `<build><plugins>` 里声明该插件的，子模块的 `pluginManagement`
   不会覆盖已继承的插件配置。改为 `<build><plugins>` 后生效。
2. 这直接印证了母仓的**教训七「门禁必须验证真的执行了」**：如果不做「把阈值改成 0.99 看它是否报错」
...
**传输底座补测试：覆盖门禁从「空转」变为真实生效（2026-09-14）**

复审发现 `ypbin-iot-transport`（Netty 底座，4 个生产类）**零测试**，
因而 JaCoCo 因「无 jacoco.exec」**静默跳过**了它——门禁在这个模块上等于空转，
而它是所有字节级协议（TCP 透传，以及后续的 UDP/WebSocket）的共同底座。

| 项 | 修复前 | 修复后 |
|---|---|---|
| 测试 | **0** | `FramingSpecTest` 6 用例 + `NettyTransportTest` 11 用例 |
| 指令覆盖 | 无数据（门禁跳过） | **81.2%** |
| 分支覆盖 | 无数据（门禁跳过） | **64.2%** |

`NettyTransportTest` 用本机 `ServerSocket` 做对端，覆盖：建链与状态翻转、关闭幂等、
`whenClosed` 完成、入站帧送达、**分隔符模式切帧（粘包不得当成一帧）**、
不可达端点快速失败、**空闲静默必须真的关链**、TLS fail-fast、
分隔符长于帧长上限的拒绝、`write` 真正到达对端、会话绑定约束、`describe`/`unwrap`、入站容量设置。

**当时各模块基线与余量（2026-09-14 记录，数字已被取代）**：

| 模块 | instr | branch | 距 0.64 下限的余量 |
|---|---|---|---|
| core | 82.8% | 66.4% | 2.4% |
| runtime | 82.6% | 69.1% | 5.1% |
| spring-boot-starter | 88.9% | 67.3% | 3.3% |
| protocol-tcp | 85.9% | 78.9% | 14.9% |
| protocol-modbus | 81.3% | **64.9%** | **0.9%（约 3 个分支）** |
| protocol-mqtt | 88.5% | 84.3% | 20.3% |
| protocol-opcua | 80.6% | 67.3% | 3.3% |
| **transport** | 81.2% | **64.2%** | **0.2%（约 1 个分支）** |

> ⚠️ **上表不是当前值，不要再据此判断「该补哪个模块」**——逐模块实测值一律看
> [`tools/generated/iot-coverage.json`](../tools/generated/iot-coverage.json)
> （由 `node tools/export-coverage.mjs` 从构建产物生成，模块集合有门禁、数值不设门禁）。
> 2026-09-15 实测：余量最薄的是 **core（+3.0pp）与 spring-boot-starter（+3.2~3.5pp）**，
> 其后是 modbus（+3.6pp）；上表里余量最薄的 **transport 实际已到 +7.7pp**（当时它刚补完测试）。
> 这正说明手抄的覆盖率快照**活不过几轮改动**，所以现在改为由构建产物生成。

> **当时的判断**：`modbus` 与 `transport` 的余量不足 1%，任何微小改动（净丢 3 个已覆盖分支
> 或净增 4 个未覆盖分支）都会让它们假红。这不是门禁太严，而是当时的覆盖率确实低——
> 结论「应补分支用例、而不是下调阈值」仍然成立，且 0.64 是**防倒退下限而非目标**。
...
**当时的分支余量（相对 0.64 下限，2026-09-14 记录，数字已被取代）**：mqtt +20.3 / tcp +14.9 / modbus +6.7 / runtime +5.6 / transport +4.9 / opcua +3.3 / starter +3.3 / core +2.4。
> 逐模块**当前**实测值一律看 [`tools/generated/iot-coverage.json`](../tools/generated/iot-coverage.json)；
> 2026-09-15 实测最薄的是 core（+3.0pp）与 starter（+3.2~3.5pp），与上面这组排序已经不同。
...
**OPC UA 信任模型说清楚 + 装配期诊断（2026-09-15）**

复审曾指出「信任目录放 CA 签发的叶子证书不可用」，当时的结论只写进了文档。
本轮读了 Milo 源码（`CertificateValidationUtil.buildTrustedCertPath` / `buildCertPath`）
把机制彻底确认，并把它变成**装配期诊断**而不是运行期一句看不懂的报错。

**机制（源码级确认）**：
- 信任锚**只能由自签证书构成**：`trustedCertificates` 里 `isSelfSigned(c)` 且（chain 恰为该证书 或 `isCa(c)`），
  外加 `issuerCertificates` 里 `isCa(c) && isSelfSigned(c)`
- 中间证书来自 `trustedCertificates` 与 `issuerCertificates` 中**非自签的 CA**
- 路径构建后还会校验「路径（含锚）中至少有一个证书在信任列表里」

**由此得出三条部署语义（务必按此配置）**：

| 信任目录内容 | 可用性 | 安全含义 |
|---|---|---|
| **服务端的自签证书** | ✅ | **精确 pin** —— 锚就是它自己，换证书必须拿到该私钥 |
| **自签 CA** | ✅ | ⚠️ **信任该 CA 签发的任何证书**，不是精确 pin |
| **CA 签发的叶子** | ❌ **不可用** | 它既不是自签（当不了锚）也不是 CA（当不了中间证书），这份配置永远用不上 |

**一个反直觉、且与安全相关的推论**：**「CA + 指定叶子」并不能得到精确 pin**。
锚是那个 CA，而锚本身就在信任列表里 → 那道「路径须含信任证书」的最终校验**恒真** →
该 CA 签发的任何证书都会通过。`SEC-16` 用两张同 CA 签发的叶子把这个结论钉住了。
**要精确 pin，只能放服务端的自签证书。**

**本轮改动**：`OpcUaSecurity.diagnoseTrustAnchors` 在装配期分类信任目录里的每张证书 ——
- 遇到「非自签、非 CA」的终端实体证书 → **显式失败**并说明该放 CA 还是自签证书
- 遇到自签 CA → **WARN** 提示它会接受该 CA 签发的一切
- 一个自签证书都没有 → 显式失败（否则每个服务端证书都会校验失败，而错误看起来像网络问题）
...
**覆盖率并发噪声：实测、部分收敛、边界如实记录（2026-09-15）**

此前只知道「覆盖率数字每次不一样 ±0.5pp」，没有量化过。本轮做了 **4 轮全量实测**，
结论如下（分支覆盖率）：

| 模块 | 4 轮区间 | 波动 | 门禁余量（对 0.64） |
|---|---|---|---|
| **runtime** | 68.30 ~ 70.62% | **2.32pp** | +4.30pp |
| spring-boot-starter | 67.15 ~ 67.52% | 0.36pp | +3.15pp |
| core / transport / protocol-tcp / protocol-modbus / protocol-mqtt / protocol-opcua | — | **0.00pp** | +2.97 ~ +20.08pp |

**噪声源定位到 4 个类的竞争态分支**（只有多线程竞争时才会走到，单线程用例几乎不命中）：

| 类 | 波动分支 | 性质 |
|---|---|---|
| `EgressRouter` | 最多（63~75） | `queuedPoints.compareAndSet` 重试、`evictOldest` 空队列、`awaitRoom` 的 sleep/中断/超时 |
| `ConnectionRegistry$Entry` | 1~2 | refCount / epoch 比较 |
| `PollingSubscriptionManager$PollingSubscription` | 1 | inFlight CAS |
| `MqttConnection` | 1 | 监听器注册/回调竞态 |

**已做的收敛**（有效但未彻底）：
- 新增 `EGRESS-11`（8 线程 × 200 次并发投放）把 `EgressRouter` 的 CAS 重试分支从「时而覆盖」
  变成「基本必然覆盖」，该类的分支覆盖由 63~68 提升到 71~75
- 4 轮实测中 `protocol-mqtt` 的波动已降为 **0.00pp**

**未能收敛的部分（如实记录）**：`runtime` 仍有 2.32pp 波动。
根因是这些分支的命中与否取决于**队列当时的满/空状态**，而这由生产者与 drain 线程的
交错决定 —— 压力测试能让「重试」必然发生，但无法保证「重试失败」「队列恰好空」这些
状态必然出现。**这类分支在原理上就无法用测试确定性钉死**（除非给生产代码加测试钩子，
那是为测试污染实现，不做）。

**一条失败的尝试也记下来**：曾想用「中断调用线程」确定性触发 `awaitRoom` 的中断分支，
写完发现**前提就是错的** —— `emit` 根本不在调用线程等待（`EGRESS-09` 的存在正是为了把等待
卸载到 drain 线程），中断调用线程碰不到那个分支。该用例已撤除，
没有留一条「看起来在测什么、其实什么都没测」的测试。

**结论与用法**：
1. **门禁是可靠的** —— 每个模块的余量都大于其自身波动（最薄的 runtime 为 1.9 倍、其余 ≥8 倍）；
2. **但精确数字不能当回归信号** —— 看到 runtime 掉了 1pp 不要当成退化，
   判断退化要看**门禁是否变红**；
3. `runtime` 的余量/波动比只有约 1.9 倍，是**需要盯着的一项**：若后续新增并发代码让
   波动继续变大，应先把波动压下来再谈覆盖率目标。

**M1 已落地协议模块（2026-09-13）**

> 下表是 **2026-09-13 当时**的记录（用例数与覆盖率都是快照）；当前值看
> [`tools/generated/iot-coverage.json`](../tools/generated/iot-coverage.json) 与各模块测试。
